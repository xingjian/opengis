 ...
 *
 */
@Path("/GeometryService/densify")
@Produces( { MediaType.TEXT_HTML, MediaType.APPLICATION_JSON })
public class DensifyResource {
	...
	public String getResult(
			...
			@QueryParam("maxSegmentLength") double maxSegmentLength) {
		return result(token, f, geometries,maxSegmentLength);
	}
	...
	public String postResult(
			...
			@FormParam("maxSegmentLength") double maxSegmentLength) {
		return result(token, f, geometries,maxSegmentLength);
	}

	private String result(String token, String f, String geometries,double maxSegmentLength) {
		String result = null;
		...
		try {
			...
			if (geometries != null && !"".equals(geometries) && maxSegmentLength>0) {
				...
				for (Iterator<Geometry> itr = geometriesList.iterator(); itr
					...
					geo = GeometryToolkit.densify(geo, maxSegmentLength);
	...
	private String generateHTMLResult(List<Geometry> resultGeometries,
			String token, String geometries, double maxSegmentLength) {
		String result = "";
		...
		try {
			...
			sb
					.append("<tr><td>Maximum Segment Length:</td><td><input type='text' name='maxSegmentLength' value='${MAX_SEGMENT_LENGTH}' /></td></tr>");
			sb
					.append("<tr><td colspan='2'><input type='submit' value='Densify' /></td></tr>");
			...
			restBody = restBody.replace("${MAX_SEGMENT_LENGTH}", maxSegmentLength <= 0 ? "" : String.valueOf(maxSegmentLength));
