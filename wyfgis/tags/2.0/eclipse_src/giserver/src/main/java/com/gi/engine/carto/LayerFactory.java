...
public class LayerFactory {
	...
	public Layer createLayer(LayerInfo layerInfo) throws Exception {
		...
		if (layerInfo != null) {
			...
			if ("file".equals(dsType)) {
				...
				layer = createLayerByFile(file, styleSource);
	...
	private Layer createLayerByFile(File file, String styleSource)
			throws Exception {
		Layer layer = null;
		...
		if (lowFileName.endsWith(".shp") || lowFileName.endsWith(".gml")
			...
			// Vector
			DataStore ds = DataStoreFactory.getDataStoreFromFile(file);
