 ...
 * 
 */
@Path("/GeometryService/intersect")
public class IntersectResource {
		...
		try {
			...
			if (geometries != null && !"".equals(geometries)
				...
				for (Iterator<Geometry> itr = geometryList.iterator(); itr
					...
					Geometry resultGeo = geo.intersection(geometryObj);
	...
	private String generateHTMLResult(List<Geometry> resultGeometries,
		...
		try {
			...
			sb
					.append("<tr valign='top'><td>Intersect Geometry:</td><td><textarea name='geometry' rows='10' cols='50'>${GEOMETRY}</textarea></td></tr>");

			sb
					.append("<tr><td colspan='2'><input type='submit' value='Intersect' /></td></tr>");
