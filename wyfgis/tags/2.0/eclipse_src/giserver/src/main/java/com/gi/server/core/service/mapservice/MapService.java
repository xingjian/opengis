...
public class MapService implements PooledService {
	...
	public MapService(String serviceRelativePath) {
		...
		String serviceDirAbsolutePath = ServiceManager.getMapServicesDir().getAbsolutePath()
				+ File.separator + realDirRelPath + File.separator;

		MapServiceDescFile mapServiceDescFile = new MapServiceDescFile(
				serviceDirAbsolutePath + ArchitectureUtil.MAP_SERVICE_DESC_FILE_NAME);
		...
		if (!started) {
			try {
				...
				sb.append(ResourceBundleManager.getResourceBundleMapServiceLog()
						.getString("ERROR.START_MAP_SERVICE"));
				sb.append("<");
	...
	public boolean stop() {
		if (started) {
			try {
				...
				StringBuilder sb = new StringBuilder();
				sb.append(ResourceBundleManager.getResourceBundleMapServiceLog()
						.getString("ERROR.STOP_MAP_SERVICE"));
				sb.append("<");
				sb.append(mapServiceDesc);
				sb.append(">");
				logger.error(sb.toString(), ex);
