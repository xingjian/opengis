...
public class StartResource {
		...
		try {
			...
			try {
				...
				if ("map".equals(str[0].toLowerCase())) {
					isSucceed = admin.startMapService(str[1]);
				}
			} catch (Exception ex) {
				ex.printStackTrace();
			}
	...
	private String generateHTMLResult(Administrator admin, String service, boolean isSucceed) {
		...
		if (!admin.isAuthorized()) {
			...
			String[] str = service.split(":");
			if (isSucceed) {
				restBody += "<b>Succeed start " + str[0] + " service '" + str[1] + "' !</b>";
			} else {
				restBody += "<b>Start " + str[0] + " service '" + str[1] + "' failed !</b>";
			}
		}

		result += AdministratorHTML.getHeader(contextRoot).replace("${TITLE}", "Start");
		result += AdministratorHTML.getNav(contextRoot).replace("${CATALOG}", " &gt; Start").replace("${HOME}",
				rootURI + "?username=" + admin.getUsername() + "&password=" + admin.getPassword());
		result += AdministratorHTML.getH2().replace("${TITLE}", "Start");
